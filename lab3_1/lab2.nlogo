...
to setup-individuals
  ...
  [
    ...
    if gender = 1 [set color black]
    if gender = 2 [set color red]
...
to go
  ask snakes [
    ...
    if count snakes-here = 2
    [
      show "2 snakes at one place"
      if not all? snakes-here [gender = 1] and not all? snakes-here [gender = 2]
      [
        show "new snake population"
        let newresistance [resistance] of max-one-of snakes-here [resistance]
